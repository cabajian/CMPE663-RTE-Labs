/**
  ******************************************************************************
  * @file           : uart.c
  * @brief          : UART handlers for the STM32L476G.
  *
  * @author         : Rick Weil (original)
  * @author         : Chris Abajian (modified)
  * @date           : September 2021
  ******************************************************************************
  */
...
/**
  * USART IRQ handler to run on interrupts generated by the USART module.
  */
...
/**
  * Captures 1 line of text from console. Terminates when CR or LF
  * are received, or "max_length" characters are exceeded. Stored
  * string is NULL terminated.
  */
void USART_Read_Line(USART_TypeDef * USARTx, uint8_t *buffer, int max_length) {
  int i = 0;
  // Iterate through each byte. Leave space for
  // 3 characters: CR, LF, NULL.
  while (i < max_length-3) {
    *buffer = USART_Read(USARTx);
    if (*buffer == '\r' || *buffer == '\n') {
      break;
    } else {
      USART_Write(USARTx, buffer++, 1);
      i++;
    }
  }
...
/**
  * Writes 1 line of text to console. Terminated at NULL terminator.
  */
void USART_Write_Line(USART_TypeDef * USARTx, uint8_t *buffer) {
  // Write until NULL character
  while (*buffer) {
    USART_Write(USARTx, buffer++, 1);
  }
}
